...
import { Leaf, Lock, Mail, ArrowRight, Sparkles, ShieldCheck } from 'lucide-react';
...
export const LoginPage = () => {
  ...
  const [email, setEmail] = useState('atharv@wastechain.org');
  ...
  const handleGoogleLogin = () => {
    setIsLoading(true);
    setTimeout(async () => {
      await login('google.user@wastechain.org', 'googlepass', true);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-3">
        ...
        <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">
          Welcome back to WasteChain
        </h2>
        <p className="text-xs text-slate-500 max-w-xs mx-auto">
          AI-powered circular economy & digital waste passport platform
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0">
        <Card className="shadow-xl border-slate-200/80 p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              label="Email Address"
              type="email"
              placeholder="user@wastechain.org"
              icon={Mail}
              value={email}
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <a href="#forgot" onClick={() => toast.info('Password reset link sent to registered email!')} className="text-emerald-700 font-semibold hover:underline">
                Forgot password?
              </a>
            </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-300 rounded-xl bg-white hover:bg-slate-50 text-slate-700 font-semibold text-sm transition-all shadow-xs"
          >
        ...
        <div className="mt-6 text-center text-xs text-slate-400">
          <span>Protected by WasteChain Verification Protocol</span>
        </div>
      </div>
    </div>
  );
